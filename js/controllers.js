...
$scope.add_flood_points_newest = function(xsec, n_total) {
  ...
  for (i=xsec.xsec.length-1;i>=1;i--) {
    ...
    var the_dist = $scope.dist({x:xsec.xsec[i].a.x, y:xsec.xsec[i].a.y},{x:xsec.xsec[i-1].a.x, y:xsec.xsec[i-1].a.y});
    if (the_dist === 0) {
      xsec.xsec.splice(i,1);  // remmove point because it's identical to neighbor
      continue;
    }
  ...
  for (i=1;i<xsec.xsec.length;i++) {
    ...
    while (accum_dist < xsec.xsec[i].dist) {
      if (xsec.xsec[i].is_bezier) {
        ...
        var bzp = $scope.bezer(xsec.xsec[i-1].a,xsec.xsec[i-1].b,xsec.xsec[i-1].c,xsec.xsec[i-1].d,t);
        x2 = bzp.x;
        y2 = bzp.y;
      } else {
...
$scope.get_scaled_point = function(clientX, clientY) {
  var offset = {};
  var point = {};
  ...
  point.x = clientX / $scope.sst2.scale + offset.x / $scope.sst2.scale;
  point.y = clientY / $scope.sst2.scale + offset.y / $scope.sst2.scale;    
  return point;
}
...
$scope.draggify = function(x, y, index, point1, point2, point3, point4) {
  point1.x = x;
  point1.y = y;
}
